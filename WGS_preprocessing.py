...
if 'bam' in input_file_format:
    ...
    rule wgs_00bam_PrepareRGwiseOutputMapForBamRevert:
        ...
        params: n="1", R="'span[hosts=1] rusage[mem=4]'", o="out/logs/WGS/rgmap.out", eo="out/logs/WGS/rgmap.err", J="create_rgmap", RG_header_list= lambda wildcards: config['input_files']['genome_personalization_module']['bam_inputs'][wildcards.sample]['read_groups']
        run:
    ...
    rule wgs_01bam_RevertToUnmappedBAM:
        ...
        params: n="16", R="'span[hosts=1] rusage[mem=10]'", java_xmx=str(16*10-4), max_records_in_ram=str(25000000), o="out/logs/WGS/revert_bam.out", eo="out/logs/WGS/revert_bam.err", J="revert_bam"
...
if 'fastq' in input_file_format:
    ...
    rule wgs_00fq_TrimAdapters:
        ...
        params: n="16", R="'span[hosts=1] rusage[mem=2]'", java_xmx=str(32), o="out/logs/WGS/trim_adapters.out", eo="out/logs/WGS/trim_adapters.err"
        conda: "envs/bwa_picard_samtools.yaml"
    ...
    rule wgs_01fq_GenerateRGWiseUbamsFromFq:
        input: fq1=lambda wildcards: config['input_files']['genome_personalization_module']['fastq_inputs'][wildcards.sample]['read_groups'][wildcards.readgroup]['R1_fq.gz'],fq2=lambda wildcards: config['input_files']['genome_personalization_module']['fastq_inputs'][wildcards.sample]['read_groups'][wildcards.readgroup]['R2_fq.gz']
        output: temp("out/WGS/{sample}.{readgroup}.unmapped.bam")
        params: n="8", R="'span[hosts=1] rusage[mem=8]'", java_xmx=str(8*8-4), o="out/logs/WGS/fastq2ubam.out", eo="out/logs/WGS/fastq2ubam.err", J="fq2ubams", \
...
rule wgs_02_BwaAndSortAndMergeBamAlignment:
    ...
    params: n="32", R="'span[hosts=1] rusage[mem=2]'", java_xmx=str(32*2), max_records_in_ram=str(250000*32*2), o="out/logs/WGS/bwa_n_mergebams_{sample}_{readgroup}.out", eo="out/logs/WGS/bwa_n_mergebams_{sample}_{readgroup}.err", J="bwa_mergebams", \
...
if 'fastq' in input_file_format:
    rule wgs_03_MergeAllRGsFQ:
        ...
        params: n="16", R="'span[hosts=1] rusage[mem=4]'", java_xmx=str(16*4), o="out/logs/WGS/merge_sample_RGs.out", eo="out/logs/WGS/merge_sample_RGs.err", J="merge_sample_RGs"
        conda: "envs/bwa_picard_samtools.yaml"
...
rule wgs_04_MarkDuplicatesAndCoordSort:
    ...
    params: n="24", R="'span[hosts=1] rusage[mem=8]'", java_xmx_dups=str(4*8), max_records_in_ram_dups=str(250000*4*8), java_xmx_sort=str(20*8), max_records_in_ram_sort=str(30000000), o="out/logs/WGS/markdups.out", eo="out/logs/WGS/markdups.err", J="mark_dups"
...
rule wgs_05_IndexBam:
    ...
    params: n="4", R="'span[hosts=1] rusage[mem=4]'", o="out/logs/WGS/index_bam.out", eo="out/logs/WGS/index_bam.err", J="index_bam"
    conda: "envs/bwa_picard_samtools.yaml"
...
rule util_CreateGenomeDict:
    ...
    params: n="1", R="'span[hosts=1] rusage[mem=16]'", o="out/logs/WGS/create_refDict.out", eo="out/logs/WGS/create_refDict.err", J="create_refDict"
    conda: "envs/gatk4.yaml"
...
rule util_CreateGenomeFastaIndex:
    ...
    params: n="1", R="'span[hosts=1] rusage[mem=18]'", o="out/logs/WGS/create_refIdx.out", eo="out/logs/WGS/create_refDict.err", J="create_refIdx"
    conda: "envs/gatk4.yaml"
...
rule util_CreateBWAindex:
    input: STOCK_GENOME_FASTA
    output: BWA_INDEX
    params: n="24", R="'span[hosts=1] rusage[mem=8]'", o="out/logs/WGS/bwa_index.out", eo="out/logs/WGS/bwa_index.err", J="create_bwa_index"
    conda: "envs/bwa_picard_samtools.yaml"
