...
&GetOptions( # required
             ...
             'max_overlap_pct=i' => \$MAX_PCT_OVERLAP,
    );
...
sub has_sufficient_overlap {
    ...
    foreach my $other_entry (@$other_entries_aref) {
        ...
        my $other_strand = $other_entry->{gene_obj}->get_strand();
        if ($strand eq $other_strand) {
