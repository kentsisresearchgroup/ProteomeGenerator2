...
main: {
    ...
    $cmd = "bash -c \"set -euxo pipefail; cat $gtf_file | egrep 'IG_V_gene|IG_C_gene|IG_D_gene|IG_J_gene' | awk '{ if (\\\$3 == \\\"exon\\\") { print } }' | egrep '^([0-9]+_)?chr14'  > $tmpdir/IGH_locus.gtf\"";
    ...
    $cmd = "bash -c \"set -euxo pipefail; cat $gtf_file | egrep 'IG_V_gene|IG_C_gene|IG_D_gene|IG_J_gene' | awk '{ if (\\\$3 == \\\"exon\\\") { print } }' | egrep '^([0-9]+_)?chr22' > $tmpdir/IGL_locus.gtf\" ";
